...
void bitbang_setup(unsigned char bitbang_pins, const bp_bitbang_speed_t speed) {
  miso_pin = (bitbang_pins == 3) ? 0b0000000010000000 : 0b0000001000000000;
...
_Bool bitbang_i2c_start(bp_bitbang_i2c_start_type_t type) {
  ...
  bitbang_set_pins_high(0b0000001000000000 + 0b0000000100000000, delay_profile->clock);

  if (type == BITBANG_I2C_RESTART) {

    bitbang_set_pins_high(0b0000001000000000, delay_profile->clock);


    bitbang_set_pins_high(0b0000000100000000, delay_profile->clock);
  }


  if ((PORTBbits.RB8 == 0) || (PORTBbits.RB9 == 0)) {
    return 1;
  }


  bitbang_set_pins_low(0b0000001000000000, delay_profile->clock);


  bitbang_set_pins_low(0b0000000100000000, delay_profile->clock);
...
void bitbang_i2c_stop(void) {
  ...
  bitbang_set_pins_low(0b0000001000000000 + 0b0000000100000000, delay_profile->clock);
...
uint16_t bitbang_read_with_write(const uint16_t value) {
  ...
  for (count = 0; count < mode_configuration.numbits; count++) {
    bitbang_set_pins(temporary & bit_index, 0b0000001000000000, delay_profile->settle);
    bitbang_set_pins_high(0b0000000100000000, delay_profile->clock);
    input = (input << 1) | bitbang_read_pin(0b0000000010000000);
    temporary <<= 1;
    bitbang_set_pins_low(0b0000000100000000, delay_profile->clock);
  }
...
uint16_t bitbang_read_value(void) {
  ...
  bitbang_read_pin(0b0000001000000000);
  value = 0;

  for (count = 0; count < mode_configuration.numbits; count++) {
    bitbang_set_pins_high(0b0000000100000000, delay_profile->clock);
    value = (value << 1) | bitbang_read_pin(0b0000001000000000);
    bitbang_set_pins_low(0b0000000100000000, delay_profile->clock);
  }
...
void bitbang_write_bit(const _Bool state) {
  ...
  bitbang_set_pins(state, 0b0000001000000000, delay_profile->settle);
...
void bitbang_set_clk(const _Bool state) {
  bitbang_set_pins(state, 0b0000000100000000, delay_profile->settle);
}

void bitbang_set_cs(const _Bool state) {
  bitbang_set_pins(state, 0b0000000001000000, delay_profile->settle);
}
