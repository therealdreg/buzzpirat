...
void pic_setup_prepare(void) {
  ...
  bitbang_set_pins_low(0b0000001000000000 | 0b0000000100000000, 100);
}
...
uint16_t read_bits(const uint8_t bits) {
  ...
  for (size_t index = 0; index < bits; index++) {
    bitbang_set_pins_high(0b0000000100000000, 100 / 2);
    data = (data >> 1) |
           ((bitbang_read_pin(0b0000001000000000) == 1 ? 1 : 0) << (1 << (bits - 1)));
    bitbang_set_pins_low(0b0000000100000000, 100 / 2);
  }

  bitbang_set_pins_high(0b0000000100000000, 100 / 2);
  bitbang_set_pins_low(0b0000000100000000, 100 / 2);
  bitbang_set_pins_low(0b0000001000000000, 100 / 5);
...
void write_bits(const uint16_t data, const uint8_t bits) {
  ...
  for (size_t index = 0; index < bits; index++) {
    bitbang_set_pins_high(0b0000000100000000, 100 / 4);
    bitbang_set_pins((data & mask) == mask, 0b0000001000000000, 100 / 4);
    bitbang_set_pins_low(0b0000000100000000, 100 / 4);
    bitbang_set_pins_low(0b0000001000000000, 100 / 4);
    mask <<= 1;
  }
}
...
void binary_io_enter_pic_mode(void) {
  ...
  for (;;) {
    ...
    case 0x00:
      ...
      case 0x10:
        if (command & 0x08) {
          bitbang_set_pins((command & 0x04) == 0x04, 0b0000010000000000, 5);
          bitbang_set_pins((command & 0x02) == 0x02, 0b0000000010000000, 5);
          bitbang_set_pins((command & 0x01) == 0x01, 0b0000000001000000, 5);
        } else {
          if (command & 0x04) {
