...
# 226 "../uart.c"
    mode_configuration.speed = getnumber(1, 1, 10, 0) - 1;

    if (mode_configuration.speed == 9) {
      bp_message_write_line(__builtin_tbladdress(MSG_UART_RAW_BRG_PROMPT_str));
...
# 299 "../uart.c"
  if (U2BRG < U1BRG) {
    bp_message_write_line(__builtin_tbladdress(MSG_UART_POSSIBLE_OVERFLOW_str));
  }
...
# 374 "../uart.c"
      if (macro == 3) {
        ...
        PORTBbits.RB8 = PORTAbits.RA4;
        PORTAbits.RA3 = PORTBbits.RB6;
      }
...
uint32_t uart_get_baud_rate(const _Bool quiet) {
  ...
  while ((PORTBbits.RB7 == 1) && (U1STAbits.URXDA == 0)) {
    __builtin_nop();
  }
