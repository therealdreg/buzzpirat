...
_Bool user_serial_transmit_done(void) { return U1STAbits.TRMT; }

_Bool user_serial_ready_to_read(void) { return U1STAbits.URXDA; }
...
void user_serial_ringbuffer_append(const char character) {
  ...
  if (user_serial_ringbuffer_write == 4096) {
...
_Bool user_serial_check_overflow(void) { return U1STAbits.OERR; }

void user_serial_clear_overflow(void) { U1STAbits.OERR = 0; }
...
void user_serial_process_transmission_interrupt() {
  ...
  if ((IEC0bits.U1TXIE == 1) || (UART1TXAvailable == UART1TXSent)) {
    return;
  }
